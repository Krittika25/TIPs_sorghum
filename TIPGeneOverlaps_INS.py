'''
This script finds the overlap between TIPs in each of the wild sorghum genotype (from insertion data) and BTx623
genes
...
def getGeno(flankfile,genotypes,gffFile,outfile):
	...
	for gindex,gname in enumerate(genotypes):
		...
		with open("/Users/krittikakrishnan/Documents/sorghum_genomes/TIPs/"+gname+"_noref.txt") as geno:
			geno.readline()
			for l2 in geno:
				t2=l2.rstrip().split("\t")
				ID=t2[0]
	...
	with open(gffFile) as gff:
		...
		for l3 in gff:
			...
			for k,v in TEcoords.items():
				#finding overlaps between TE and gene coordinates in BTx623 with 1000bp flanking; since the TE is inserted only in the wild genotype
				#reference only has a single coordinate for which the flanking is calculated
				if (v[0]==t3[0]) and (int(t3[3])-1000 <= int(v[1]) and int(t3[3])+1000 >= int(v[2])) and (t3[2] in "gene"):
...
def main():
	...
	infile="/Users/krittikakrishnan/Documents/sorghum_genomes/TIPs/BTx623.Non-referenceTEinsertions_and_flanking1kb.info"
	outfile="/Users/krittikakrishnan/Documents/sorghum_genomes/TIPs/TIPOverlaps_INS.out"
